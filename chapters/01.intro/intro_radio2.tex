...
However, this would reduce the effective resolution of the reconstruction. We can solve the ill-posed inverse problem and retrieve the observed image at a higher resolution than possible with the Nyquist-Shannon sampling theorem. We solve the ill-posed inverse problem by including prior information about the observed image. We use numerical optimization algorithms to find the most likely image, which is consistent with the measurements and consistent with our prior information. This is known in signal processing as compressed sensing \cite{candes2006robust,donoho2006compressed}. The theory of Compressed Sensing shows that, under the right prior information, we are guaranteed to reconstruct the observed image at a higher resolution than under the Nyquist-Shannon sampling theorem.
...
We introduce the theory of Compressed Sensing for the problem of radio interferometric image reconstruction. 

T

We solve a numerical optimization problem. The theory of Compressed Sensing tells us how likely we are to reconstruct the observed image this way. As we will see later, the theory guarantees us exact reconstruction under certain assumptions. But the difficulty is that we cannot verify these assumptions in practice.
...
We reconstruct the image by finding the optimum of \eqref{radio:cs:l2}.
It is convex, meaning only has one, global optimum.
Convex optimization algorithms that find the optimum of the function.
 But since the measurements are incomplete, the minimum of the function \eqref{radio:cs:l2} does not point to the observed image.
Only when we fulfill the Nyquist-Shannon rate, only then does the function \eqref{radio:cs:l2} point to the observed image.

We know things about the image.
For radio astronomy, we know the image will likely contain stars. Stars are emissions which are concentrated around a single pixel. Most pixels of the image will be zero, except for the locations where the interferometer has located stars. In other words, we know that the image is sparse. We can add a regularization to the objective function \eqref{radio:cs:l2} and force the reconstruction to be sparse. This results in the objective 
...
LASSO objective
%Data term $\| V - Fx \right \|_2^2$, and two names for the second term ($\left \| x \right \|_1$). We can call the second term a prior when we look at it from the bayesian statistics. 
%View from optimization, \eqref{radio:cs:lasso} is the LASSO objective and $\left \| x \right \|_1$ is the regularization. We reduce the possiblity space.
Still convex
Regularization as the view from optimization.
Prior information from the view of Bayesian statistics.

Where does the LASSO objective point to? Does it point to the observed image? Empirically, this is often the case. Of course only when our prior is right. For example when we have extended emissions, when the interferometer picked up clouds of hydrogen, LASSO is a bad prior and it may point to a rubbish image.
...
That is why we give first an intuitive explanation on why compressed sensing works, and why it give sense to the fact that we do not need as many samples as Nyquist-Shannon describes.

Then we show 

\subsubsection{Intuitive explanation of Compressed Sensing}
Intuitive
