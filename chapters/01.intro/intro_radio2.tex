...
This section gives an introduction to radio interferometric imaging. We first describe how a radio interferometer measures visibilities. We then discuss how a reconstruction algorithm works in theory with a simplified view of the visibility measurements and show how real-world visibilities are more complicated. Lastly, we show how real-world reconstruction algorithm deal with real-world visibility measurements, and how they work in practice.
...
In short, the MeerKAT radio interferometer measures a large volume of visibilities. We generally have more visibility measurements than pixels in the reconstruction. Still, the measurements are incomplete, and from the measurements alone, we cannot reconstruct the observed image. In the literature this is known as an ill-posed inverse problem. We wish to find the observed image from measurements in the Fourier space, but the measurements alone do not contain all the relevant information. We can calculate the inverse Fourier transform of the visibility measurements, but it does not result in the observed image. However, we can include prior information about the image, we can reconstruct the most-likely image given the measurements.

Describe how we can reconstruct the most-likely image, what a reconstruction algorithm does in theory. 
Then discuss how real-world effects from the interferometer, like noise and calibration complicates the reconstruction algorithm.
Then introduce how reconstruction algorithms for radio interferometers work in practice reconstruction algorithms in practice

\subsection{Reconstructing the most-likely image}
Here we introduce how a reconstruction can find the most-likely image given the measurements. First, we formulate the image reconstruction as a minimization problem with the following objective function:
...
Still an open problem for the data volume of MeerKAT.
Focus on the numerical optimization algorithm and how it deals with the large data volume of MeerKAT.

A reconstruction algorithm consists of three parts: A regularization that models what a plausible image looks like, an objective function and a numerical optimization algorithm. In this project we focus on the numerical optimization algorithm, and try to speed up the 
