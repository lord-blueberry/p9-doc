...
We introduce the theory of Compressed Sensing for the problem of radio interferometric image reconstruction. We show how to formulate the image reconstruction as an optimization problem, how we can include prior information and what guarantees arise that we actually solve the ill-posed inverse problem. We will see that we can reconstruct the observed image from incomplete measurements, below the Nyquist-Shannon sampling rate.

\begin{equation}\label{radio:cs:linear}
\underset{x}{solve}\quad V - Fx = 0
\end{equation}

System of linear equations.
Show the ill-posed inverse problem at this.

Nyquist-shannon rate. Twice as many Visibilities, at uniformly spaced $u$ and $v$ locations.
In that case, we can simply calculate the inverse Fourier transform.
We do not have that. 
We do have a large number of visibilities, generally several times larger than the number of pixels. But as we saw in Figure \ref{radio:sampling:pattern}, the sampling pattern is not uniform. We are missing samples at important locations. 
This also means we are measuring redundant information.

Over-determined problem. We have more visibilities than pixels.
But we also have noise.
Amplitude and phase of the visibilities. More details in later section.
...
Now with \eqref{radio:cs:l2} we have "solved" the noise issue.
Convex optimization problem.
Has a global minimum.
Convex optimization algorithms that give us the global minimum of \eqref{radio:cs:l2}.


Is the observed image the the global minimum? Sadly, we do not have any guarantee to find it there.

The crucial point for compressed sensing: We need to add prior information. We want to penalize unlikely images and find the optimum between being close to the data, and being consistent with prior information.
In the case of radio astronomy, we can assume that the image will contain stars. Stars are emissions which are concentrated around a single pixel.
We want to say that only a few pixels are likely to be non-zero. Most of the image will be zero, only at a few locations where the stars have to be. 
...
This is the simplest prior, but it gets used in real-world applications. There are other, more complex regularizations which we will discuss later.

How many samples do we need, untli we find the observed image at the optimum of \eqref{radio:cs:lasso}? 
Nyquist-Shannon rate tells us it depends on the number of pixels in the image. The theory of compressed sensing gives us guarantees depending on the number of non-zero pixels.
