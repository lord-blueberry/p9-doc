...
This section gives a short introduction on the theory of compressed sensing. It tries to give an intuition on why we find the truly observed image at the optimum of the function \eqref{intro:linear:compressed}.
...
With wide ranging implications for signal processing tasks like de-nosisng, compression and reconstruction.
...
To prove exact reconstruction of signals, the matrix $F$ needs to fulfill the restricted isometry property.
...
Active field of research, how we prove exact reconstruction in theory \cite{candes2011probabilistic}. For our project, it is important for designing theoretically optimal instruments. Influences how the antenna layout of an instrument is planned. This project is focused on the reconstruction algorithm, the antenna layout is given

Learning sparse spaces


In practice, it is virtually impossible to validate these assumptions. Meaning for a given observation, we do not know if the 
Theoretical part, since it is virtually impossible to validate the necessary assumptions in practice.

Compressed sensing based reconstruction algorithm.
