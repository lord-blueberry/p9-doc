...
\subsection{The major/minor cycle architecture for image reconstruction }\label{intro:major}
...
The major/minor cycle architecture shown in figure \ref{intro:major:fig} consists of two parts: The minor cycle, which iteratively deconvolves the dirty image with the $PSF$ (it minimizes \eqref{intro:major:reformulation:deconv}). The major cycle is more complex. The first half of the major cycle approximates the dirty image $I_{dirty} \approx F^{-1}V$, while the second half
