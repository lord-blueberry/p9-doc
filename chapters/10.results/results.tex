...
Withing 0.0007 of original. Wider range, and correlates with the psf fraction. Close to the original.
Convergence rate. We converge faster, but not to the same optimum

Speedup

Major cycles needed. one more

Similar to the other method. we start out converging faster. But we do not end in the same optimum. Natural question: what happens when we combine these two methods, can we achieve the best from both worlds?
...
Two different approximations. The errors may not be correlated, meaning we can remove the error introduced from one method with the other method.

Combine the method. Start out with an approximate deconvolution (method 2) and then switch to an approximate gradient update (method 1). This way we want to combine the faster convergence rate of method 2 with converging to the same optimum of method 1.
...
\begin{figure}[h]
	...
	\caption{Comparison of the two methods using $\frac{1}{16}$.}
