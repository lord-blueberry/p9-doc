...
Multi-scale CLEAN detects all extended emissions (we have non-zero pixels in the model for every source). It has the smallest residual magnitudes of the three algorithms, and has barely visible structures left in the residual image. Multi-scale CLEAN has reconstructed the image close to the noise level. The serial coordinate descent algorithm was run with a regularization parameter $\lambda = 1.0 * maxLipschitz$ and a L1 to L2 weighting parameter $\alpha = 0.01$. The regularization is weighted in favor of the L2 norm, which forces the serial coordinate descent algorithm to connect neighboring non-zero pixels together into a single extended emission. The serial coordinate descent reconstruction does not contain any non-zero pixels in the model image. Its residual image contains still visible structures, which means it has considered part of the real radio emission as noise. The MORESANE reconstruction surprisingly has the largest magnitudes in the residual image. It still has clear structures belonging to the extended sources in the residuals. Also note that MORESANE has not detected the top-right extended emission. It is missing in the model image.
