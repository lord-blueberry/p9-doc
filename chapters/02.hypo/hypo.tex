...
New Interferometer produce an ever increasing number of measurements, creating ever larger reconstruction problems. A single image can contain several terabytes of Fourier measurements. Handling reconstruction problems of this size forces us to use distributed computing. However, state-of-the-art Gridding and Deconvolution algorithms only allow for limited distribution. How to scale the Gridding and Deconvolution algorithms to large scale is still an open problem.

Recent developments make a distributed Gridder and a distributed Deconvolution algorithm possible. Veeneboer et al\cite{veenboer2017image} found a way to split the input and perform the Gridding on the GPU. The same splitting can potentially be used to distribute the Gridding onto multiple machines. For the Deconvolution algorithm, the theory of Compressed Sensing has lead to new designs which can produce higher quality reconstructions\cite{girard2015sparse} than state-of-the-art deconvolutions. 
