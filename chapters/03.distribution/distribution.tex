...
This is not an issue for convergence. The Lipschitz constant of the edge pixels is always lower than the center. The Lipschitz constant describes the largest step we can take without overshooting the target. We can always make smaller steps, but may pay it with more iterations. Luckily, there is a way to re-use intermediate results, and efficiently calculate the Lipschitz constant for each pixel.
